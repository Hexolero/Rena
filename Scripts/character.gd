...
const ILYA_CAMERAPOS = Vector3(0, 2.36, 0.65) # this is when camera container is a child of ilya
const ILYA_CAMERA_ROT = PI

const RENA_CAMERAPOS = Vector3(0, 0.92, 1.42) # this is when camera container is a child of rena
const RENA_CAMERA_ROT = PI
...
func _physics_process(delta):
	# check if player intends to swap characters
	if characterSwapQueued:
		characterSwapQueued = false # consume event
		if is_friend_in_sight():
			print("CHARACTER_SWAP INITIATED")
			$rena.rotate_y(PI / 2)
	
	if currentCharacter == "Swapping":
		return # do nothing physics-wise with the characters while swapping
	if currentCharacter == "Ilya":
		...
		$ilya.move_and_slide(ILYA_MOVESPEED * movementDir)
